...
var
  apiUtil = {
    ...
    getHostname : function(req) {
      ...
      return _.trim(hostname).toLowerCase().replace(/^http(s)?:\/\//, '').replace(/:[\d]+$/, '');
    },
    ...
    parseValues: function(req, name) {
      ...
      var valuesArray = _.map(bodyParams, function(element){
        ...
        // Merge properties of the element into req.options.values, omitting the blacklist
      ...
      // Check each value if it is a collection value and if yes clean it
      _.each(values, function(value, name) {
        if (!_.isUndefined(Model.attributes) && !_.isUndefined(Model.attributes[name]) && !_.isUndefined(Model.attributes[name].collection) && !_.isUndefined(Model.attributes[name].via)) {
          // Clean collection values
          var collectionIds = [];
          if (value) {
            if (_.isArray(value) && value.length) {
              _.forEach(value, function(v) {
                if (_.isFinite(parseInt(v, 10))) {
                  collectionIds.push(parseInt(v, 10));
                }
              });
            } else if (_.isString(value) || _.isNumber(value)) {
              if (value.toString().indexOf(',') > -1) {
                var valueSplitted = value.split(',');
                _.forEach(valueSplitted, function(splitted) {
                  if (_.isFinite(parseInt(splitted, 10))) {
                    collectionIds.push(parseInt(splitted, 10));
                  }
                });
              } else if (_.isFinite(parseInt(value, 10))) {
                collectionIds.push(parseInt(value, 10));
              }
            }
          }

          values[name] = collectionIds;
        }
      });
     ...
     *
     * Structure in an url:
     * `field`=(`criteria modifier`)?`value` e.g. http(s)://...?`id=2`, http(s)://...?`id==2`, http(s)://...?`id=!2`, http(s)://...?`name=<=10`
     *
    ...
    parseCriteria: function(req, name){
      ...
      // Merge `primaryKeyCriteria` and `where` and return
      where = _.merge({}, primaryKeyCriteria, where) || {};

      // Remove certain criteria
      if (where && req.options && _.isArray(req.options.removeWhere) && req.options.removeWhere.length) {
        _.forEach(where, function(value, name) {
          if (req.options.removeWhere.indexOf(name) !== -1 || req.options.removeWhere.indexOf(Model.identity + '.' + name) !== -1) {
            delete where[name];
          }
        });
      }
      ...
      for (var name in where) {
        ...
        if (_.isString(name) && _.trim(name)!='' && !_.isUndefined(Model.attributes) && !_.isUndefined(Model.attributes[name]) && !_.isFunction(Model.attributes[name]) && _.isObject(Model.attributes[name])){

          newWhere[name] = where[name];
        }
      }
      where = newWhere;
      ...
      for (var name in where) {
        ...
        if ((modifier===false || (_.isString(modifier) && modifier=='not') || _.isArray(modifier)) && whereValue.match(/,/)){
          ...
          for (var index in whereValueArray) {
            ...
            else if (whereValueArray[index].toLowerCase()=='null' || whereValueArray[index].toLowerCase()=='undefined' || whereValueArray[index]==''){

              whereValueArray[index] = null;
            }
            else if (!_.isUndefined(Model.attributes) && !_.isUndefined(Model.attributes[name]) && !_.isUndefined(Model.attributes[name].type) && _.isString(Model.attributes[name].type) && (Model.attributes[name].type.toLowerCase()=='date' || Model.attributes[name].type.toLowerCase()=='datetime'))
            {
              var
          ...
          if (whereValue.toLowerCase()=='false'){
          ...
          {
            var
              date = helper.tryNewDateFromUnixTimestamp(whereValue) || helper.tryNewDateFromString(whereValue);
    ...
    parsePage: function(req, name){
      ...
      else if (!_.isUndefined(Model['_config']) && !_.isUndefined(Model['_config'].defaultPage) && !isNaN(parseInt(Model['_config'].defaultPage)) && parseInt(Model['_config'].defaultPage)>=1){

        page = parseInt(Model['_config'].defaultPage);
      }
    ...
    getMaximumLimit: function(req, name){
      ...
      if (!_.isUndefined(Model['_config']) && !_.isUndefined(Model['_config'].maximumLimit) && (Model['_config'].maximumLimit===false || (!isNaN(parseInt(Model['_config'].maximumLimit)) && parseInt(Model['_config'].maximumLimit)>=1))){

        maximumLimit = Model['_config'].maximumLimit!==false ? parseInt(Model['_config'].maximumLimit) : false;
      }
    ...
    parseLimit: function(req, name){
      ...
      else if (!_.isUndefined(Model['_config']) && !_.isUndefined(Model['_config'].defaultLimit) && !isNaN(parseInt(Model['_config'].defaultLimit)) && parseInt(Model['_config'].defaultLimit)>=1){

        limit = parseInt(Model['_config'].defaultLimit);
      }
    ...
    parsePopulate: function(req, name){
      ...
      else if (!_.isUndefined(Model['_config']) && !_.isUndefined(Model['_config'].defaultPopulate) && (_.isString(Model['_config'].defaultPopulate) || _.isBoolean(Model['_config'].defaultPopulate) || _.isArray(Model['_config'].defaultPopulate))){

        populate = _.isString(Model['_config'].defaultPopulate) ? Model['_config'].defaultPopulate.split(',') : Model['_config'].defaultPopulate;
      }
    ...
    getMaximumPopulateLimit: function(req, name){
      ...
      if (!_.isUndefined(Model['_config']) && !_.isUndefined(Model['_config'].maximumPopulateLimit) && (Model['_config'].maximumPopulateLimit===false || (!isNaN(parseInt(Model['_config'].maximumPopulateLimit)) && parseInt(Model['_config'].maximumPopulateLimit)>=1))){

        maximumPopulateLimit = Model['_config'].maximumPopulateLimit!==false ? parseInt(Model['_config'].maximumPopulateLimit) : false;
      }
    ...
    parsePopulateLimit: function(req, name){
      ...
      else if (!_.isUndefined(Model['_config']) && !_.isUndefined(Model['_config'].defaultPopulateLimit) && !isNaN(parseInt(Model['_config'].defaultPopulateLimit)) && parseInt(Model['_config'].defaultPopulateLimit)>=1){

        populateLimit = parseInt(Model['_config'].defaultPopulateLimit);
      }
    ...
    parseSort: function(req, name){
      ...
      else if (!_.isUndefined(Model['_config']) && !_.isUndefined(Model['_config'].defaultSort) && _.isString(Model['_config'].defaultSort) && _.trim(Model['_config'].defaultSort)!=''){

        sort = _.trim(Model['_config'].defaultSort);
      }
    ...
    parseFields: function(req, name){
      ...
      else if (!_.isUndefined(Model['_config']) && !_.isUndefined(Model['_config'].defaultFields) && _.isString(Model['_config'].defaultFields) && _.trim(Model['_config'].defaultFields)!=''){

        fields = _.trim(Model['_config'].defaultFields);
      }
      ...
      // If select array is empty or have the same number of elements as the select array master reset it
      if (!_.size(selectArray) || (_.size(selectArray) && _.size(selectArray) === _.size(selectArrayMaster))) {
        selectArray = selectArrayMaster;
      }

      // Remove certain fields
      if (req.options && _.isArray(req.options.remove) && req.options.remove.length) {
        _.forEach(req.options.remove, function(key) {
          var
            selectArrayRemoveIndex = selectArray.indexOf(key);

          if (selectArrayRemoveIndex !== -1) {
            selectArray.splice(selectArrayRemoveIndex, 1);
          }
        });
      }
    ...
    getPaginationUrls : function(req, countResults, name){
      ...
      var
        ...
        urlAfterPage = self.parseLimitToString(req)+(parameters!='' ? (self.parseLimitToString(req)!='' ? '&' : '')+parameters : '');
